...
public class ClickGUI extends GuiScreen {
    ...
    private final ModuleDetailPanel detailPanel;
    ...
    private final int scrollBarColor = new Color(100, 150, 255, 150).getRGB();
    private final int scrollBarHoverColor = new Color(120, 170, 255, 200).getRGB();
    private final int scrollBarBackgroundColor = new Color(30, 45, 70, 100).getRGB();
    ...
    // 按钮坐标和尺寸
    private int dragButtonX = 0;
    private int dragButtonY = 0;
    private final int dragButtonW = 80;
    private final int dragButtonH = 24;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        // 绘制右上角按钮
        ScaledResolution sr = new ScaledResolution(mc);
        dragButtonX = sr.getScaledWidth() - dragButtonW - 20;
        dragButtonY = 20;
        boolean dragBtnHovered = mouseX >= dragButtonX && mouseX <= dragButtonX + dragButtonW && mouseY >= dragButtonY && mouseY <= dragButtonY + dragButtonH;
        int btnColor = dragBtnHovered ? HOVER_COLOR : SECONDARY_COLOR;
        RenderUtil.drawRect(dragButtonX, dragButtonY, dragButtonX + dragButtonW, dragButtonY + dragButtonH, btnColor);
        RenderUtil.drawOutline(dragButtonX, dragButtonY, dragButtonW, dragButtonH, PRIMARY_COLOR);
        // Drag按钮文字
        String btnText = "拖动UI位置";
        int textWidth = FontUtil.font18.getStringWidth(btnText);
        FontUtil.font18.drawString(btnText, dragButtonX + (dragButtonW - textWidth) / 2, dragButtonY + (dragButtonH - 8) / 2, TEXT_COLOR);
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        // 检查是否点击了右上角按钮
        if (mouseButton == 0 && mouseX >= dragButtonX && mouseX <= dragButtonX + dragButtonW && mouseY >= dragButtonY && mouseY <= dragButtonY + dragButtonH) {
    ...
    private static class SearchBox {
        ...
        public void draw(int mouseX, int mouseY) {
            ...
            drawSearchIcon(iconX, iconY, iconSize);
        ...
        private void drawSearchIcon(int x, int y, int size) {
            ...
            // 绘制圆形
            RenderUtil.drawCircle(centerX, centerY, radius, TEXT_COLOR);
            ...
            RenderUtil.drawHorizontalLine(handleX, handleX + handleLength, handleY, ClickGUI.TEXT_COLOR);
            RenderUtil.drawVerticalLine(handleX + handleLength, handleY, handleY + handleLength * 0.3f, ClickGUI.TEXT_COLOR);
        }
    ...
    private class CategoryPanel {
        ...
        private void drawHeader(int mouseX, int mouseY) {
            ...
            int nameWidth = FontUtil.font26.getStringWidth(categoryName);
            FontUtil.font26.drawString(categoryName, x + (width - nameWidth) / 2, y + 5, TEXT_COLOR);
            // 展开/收起图标
